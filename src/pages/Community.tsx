...
function Community() {
  ...
  const [commentModalMessage, setCommentModalMessage] = useState<string>("");
  const [commentModalUserName, setCommentModalUserName] = useState<string>("");
  const [commentModalPostType, setCommentModalPostType] = useState<string>("");
  const [commentModalPhotoUrl, setCommentModalPhotoUrl] = useState<string>("");
  const [commentModalImgSrc, setCommentModalImgSrc] = useState<string>("");
  ...
  const handleCardClick = (post : any) => {
    ...
    setCommentModalMessage(post.data.message);
    setCommentModalUserName(post.data.userName);
    setCommentModalPostType(post.data.postType);
    setComments(post.data.comments);
    setCommentModalPhotoUrl(post.data.photoURL);
                   ...
                   <IonItem lines='none' >
                      <IonLabel class="ion-text-wrap">
                        ...
                        <div className="ion-img-container">
                          <br></br>
                          <IonImg  onClick={() => {PhotoViewer.show(comment.imgSrc)}} src={comment.imgSrc}/>
                        </div>
                        ) : null}
                      </IonLabel>
                      <div></div>
          ...
          {topPosts && topPosts.length > 0 ? (
            topPosts.map((post) => {
              ...
              <SwiperSlide key={post.key}>
                <IonCard  onClick={() => {handleCardClick(post);}} className='ion-card-community' mode='ios'>
                <IonCardContent>
                {post.data.postType && post.data.postType != "general" ? 
                        (
                          <IonFab horizontal='end'>
                            <p style={{fontWeight:"bold", color:getColor(post.data.postType)}}>{post.data.postType.toUpperCase()}</p>
                          </IonFab>
                        ) 
                        : (null) }
                    ...
                    {post.imgSrc && post.imgSrc.length > 0? (
                      <div>
                        <br></br>
                      <IonImg className='ion-img-style' src={post.imgSrc} />
                      <br></br><br></br><br></br>
                      </div>
                    ) : (null)}
